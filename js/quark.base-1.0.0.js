...
{	
	...
	var copy = this.children.slice(0);
	//trace("Stage.prototype._update :    "+copy.length+copy[0]);
	for(var i = 0, len = copy.length; i < len; i++)
	{
		var child = copy[i];
		//trace('stage child:'+child);
		child._depth = i + 1;
		child._update(timeInfo);
	}
...
{
	//trace("MovieClip.prototype.render this.currentFrame"+this.currentFrame);
...
(function(){
    ...
    var TextBox = Quark.TextBox = function(props){
        ...
        this.textBox = new Q.createDOM('input',{type:'text',value:defaultValue});
        this.textBox.style.cssText = textBoxStyle;
        var obj = new Q.DisplayObject(childProps);
        var drawable = new Quark.Drawable(this.textBox,true);
        obj.drawable =drawable;
        obj.eventEnabled = true;
        this.addChild(obj);
        var self = this;
        this.textBox.addEventListener(events[2],function(){
            self.textBox.focus();
        });
    };
...
(function(){
    ...
    var PasswordBox = Quark.PasswordBox = function(props){
        this.id = Quark.UIDUtil.createUID("Input");
        //去除x,y值
        var thisProps = {x:0,y:0,width:200,height:500};
        var childProps = {x:0,y:0,width:200,height:50};
        ...
        PasswordBox.superClass.constructor.call(this, thisProps);

        this.textBox = new Q.createDOM('input',{type:'password',value:defaultValue});
        this.textBox.style.cssText = textBoxStyle;
        var obj = new Q.DisplayObject(childProps);
        var drawable = new Quark.Drawable(this.textBox,true);
        obj.drawable =drawable;
    ...
    Quark.inherit(PasswordBox, Quark.DisplayObjectContainer);
...
(function(){
    var TextArea  = Quark.TextArea = function(props){
        this.id = Quark.UIDUtil.createUID("textarea");
        //去除x,y值
        var thisProps = {x:0,y:0,width:200,height:500};
        var childProps = {x:0,y:0,width:200,height:50};
        var textBoxStyle = "";
        ...
        TextArea.superClass.constructor.call(this, thisProps);

        this.textArea = new Q.createDOM('textarea',{value:defaultValue});
        this.textArea.style.cssText = textBoxStyle;
        var obj = new Q.DisplayObject(childProps);
        var drawable = new Quark.Drawable(this.textArea,true);
        obj.drawable =drawable;
        obj.eventEnabled = true;
        this.addChild(obj);

        var self = this;
        this.textArea.addEventListener(events[2],function(){
            self.textArea.focus();
        });
    };
    Quark.inherit(TextArea, Quark.DisplayObjectContainer);
